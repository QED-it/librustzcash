...
/// Orchard `SighashInfo` for V1:
/// sighashInfo = (\[sighashVersion\] || associatedData) = (\[1\] || [])
const ORCHARD_SIGHASH_INFO_V1: [u8; 1] = [1]; //TODO: need to implement this sighash version properly
...
pub(crate) fn orchard_sighash_kind_to_info(kind: &OrchardSighashKind) -> Vec<u8> {
    match kind {
        ...
        OrchardSighashKind::OnlyActionGroup => ORCHARD_SIGHASH_INFO_V1.to_vec(),
    }
}
