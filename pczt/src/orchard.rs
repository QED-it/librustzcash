...
impl Bundle {
    pub(crate) fn into_parsed(
        self,
    ) -> Result<orchard::pczt::Bundle<OrchardVanilla>, orchard::pczt::ParseError> {
        let actions = self
            .actions
            .into_iter()
            .map(|action| {
                let spend = orchard::pczt::Spend::parse(
                    ...
                    action.spend.rho,
                    action.spend.rseed,
                    None, // TODO rseed_split_note
                    action.spend.fvk,
                    action.spend.witness,
                    action.spend.alpha,
                    None, // TODO split_flag
                    action
                        .spend
                        .zip32_derivation
                        .map(|z| {
    ...
    pub(crate) fn serialize_from(bundle: orchard::pczt::Bundle<OrchardVanilla>) -> Self {
        let actions = bundle
            .actions()
            .iter()
            .map(|action| {
